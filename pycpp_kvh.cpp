...
py::object kvh_read(std::string fn, const std::string& comment_str="", const bool strip_white=false, const bool skip_blank=false, const std::string& split_str="", const bool follow_url=false) {
    if (fn.size() == 0)
        throw py::value_error("kvh_read: file name is empty");
    ...
    if (comment_str.find('\t') < std::string::npos || comment_str.find('\n') < std::string::npos) {
        throw py::value_error("kvh_read: parameter 'comment_str' cannot have tabulation or new line characters in it");
    }
...
PYBIND11_MODULE(ckvh, m) {
    ...
    m.def("kvh_read",  ks, "fn"_a, "comment_str"_a="", "strip_white"_a=false, "skip_blank"_a=false, "split_str"_a="", "follow_url"_a=false);
}
